...
setColors = ->
  ...
  root.style.setProperty('--uno-1', chroma.mix( uno, 'hsl(250, 100%, 96%)', .75))  # how much white
  root.style.setProperty('--uno-2', uno)                                            # <- set by user
  root.style.setProperty('--uno-3', chroma.mix( uno, 'hsl(250, 6%, 18%)', .5))    # how much desaturated background (@syntax-bg)
